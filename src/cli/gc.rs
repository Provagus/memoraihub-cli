...
pub fn run(args: GcArgs, config: &Config) -> Result<()> {
    ...
    if preview.candidates.is_empty() {
        println!("✨ No deprecated/superseded facts older than {} days.", retention_days);
        return Ok(());
    }
    
    println!("🧹 Found {} fact(s) to clean up (older than {} days):\n", 
             preview.candidates.len(), retention_days);
