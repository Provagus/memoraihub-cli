...
fn approve_one(storage: Option<&Storage>, queue: &PendingQueue, id_str: &str, config: &Config) -> Result<()> {
...
fn approve_all(storage: Option<&Storage>, queue: &PendingQueue, skip_confirm: bool, config: &Config) -> Result<()> {
    ...
    if !skip_confirm {
        ...
        for item in &queue_pending {
            println!("  {} → {} {}", format_queue_id(&item.id), item.target_kb, item.path);
        }
...
fn push_to_remote(item: &PendingWrite, config: &Config) -> Result<()> {
    ...
    let kb_config = config.get_kb(&item.target_kb)
        .ok_or_else(|| anyhow::anyhow!("KB '{}' not found in config", item.target_kb))?;
    
    let url = kb_config.url.as_ref()
    ...
    let (endpoint, payload) = match item.write_type {
        ...
        PendingWriteType::Correct => {
            let endpoint = format!("{}/api/v1/facts/{}/correct", url, item.supersedes.as_deref().unwrap_or(""));
            let payload = serde_json::json!({
        ...
        PendingWriteType::Extend => {
            let endpoint = format!("{}/api/v1/facts/{}/extend", url, item.extends.as_deref().unwrap_or(""));
            let payload = serde_json::json!({
...
fn parse_meh_id(id_str: &str) -> Result<Ulid> {
    ...
    Ulid::from_string(clean)
        .map_err(|e| anyhow::anyhow!("Invalid ID '{}': {}", id_str, e))
}
